    ...
    public partial class Form1 : Form
    {
        ...
        private bool IsServiceRunning(string path)
        {
            ...
            foreach (var process in Process.GetProcessesByName(processName))
            {
                ...
                if (process.MainModule != null)
                {
                    ...
                    if (fileName != null && fileName.Equals(path, StringComparison.OrdinalIgnoreCase))
                    {
                        return true;
                    }
                }
        ...
        private void StartProcessMysql_OLDS(string workingDirectory, string command)
        {
            ProcessStartInfo startInfo = new ProcessStartInfo
            {
                ...
                Arguments = $"/C \"{command}\"",
                WorkingDirectory = workingDirectory,
                UseShellExecute = true, // Usa o shell do sistema (cmd.exe)
                WindowStyle = ProcessWindowStyle.Normal, // Abre o terminal de forma visível
            };
            ...
            try
            {
                using (Process process = Process.Start(startInfo))
                {
                    process.WaitForExit(); // Opcional: Aguarda o término do processo
                }
            }
            catch (Exception ex)
            {
        ...
        private void StopProcessMysql()
        {
            try
            {
                // Obtém todos os processos que têm o nome "mysqld"
                Process[] mysqlProcesses = Process.GetProcessesByName("mysqld");

                // Verifica se algum processo foi encontrado
                if (mysqlProcesses.Length > 0)
                {
                    foreach (Process process in mysqlProcesses)
                    {
                        process.Kill(); // Termina o processo
                        process.WaitForExit(); // Aguarda o término completo do processo
                    }

                    MessageBox.Show("MySQL foi parado com sucesso.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Nenhum processo MySQL foi encontrado em execução.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Erro ao parar o MySQL: {ex.Message}", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
